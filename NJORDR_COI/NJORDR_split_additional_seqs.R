...
additional_seqs <- read.table(opt$additional_seqs,header = F,quote = NULL,sep = '\t', fill = TRUE)
# additional_seqs <- read.table("Additional_seqs.tsv",header = F,quote = NULL,sep = '\t',fill = TRUE)
...
additional_seqs <- additional_seqs[,c(1,3,6)]
additional_seqs <- additional_seqs[additional_seqs$V6!="",]

write.table(additional_seqs, paste0(directory,"/seqs_2join.tsv"),row.names = F,col.names = F,quote = F,sep = "\t")
